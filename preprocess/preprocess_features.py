...
def run_batch(cur_batch, model):
    ...
    with torch.no_grad():
        feats = model.encode_image(image_batch).float()
...
def extract_clips_with_consecutive_frames(path, num_clips, num_frames_per_clip):
    ...
    try:
        ...
        if args.model == 'resnext101':
            ...
            return list(np.zeros(shape=(num_clips, num_frames_per_clip, 1, 3, 224, 224))), valid
...
def generate_h5(model, video_ids, num_clips, outfile, device):
    """
    Args:
...
if __name__ == '__main__':
    ...
    # network params
    parser.add_argument('--model', default='resnet101', choices=['clip_image', 'resnet101', 'resnext101'], type=str)

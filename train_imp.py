...
from model.TempAligner_extended import TempAligner
...
def train(cfg, args):
    ...
    tempaligner = TempAligner(**model_kwargs).to(device)
    ...
    for name, param in tempaligner.named_parameters():
    ...
    model_kwargs_tosave = {
        'tempaligner': temp_ckpt,
        'semanticaligner': semantic_ckpt
    }

    for param in semanticaligner.parameters():
        param.requires_grad = False

    optimizer = optim.Adam(tempaligner.video_summarizer.parameters(), lr=cfg.train.lr)
    ###################################################################################
    ###################################################################################
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        tempaligner.video_summarizer.train()
        total_acc, count = 0, 0
        batch_mse_sum = 0.0
        ...
        if True:
            ...
            save_checkpoint(epoch, tempaligner.video_summarizer, optimizer, model_kwargs_tosave, os.path.join( ckpt_dir, 'videosummarizer_{}.pt'.format(epoch) ) )
            sys.stdout.write('\n >>>>>> save to %s <<<<<< \n' % (ckpt_dir))
            sys.stdout.flush()
